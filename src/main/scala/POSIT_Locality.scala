...
class POSIT_Locality(coalesce: Boolean) extends Module {
 ...
 // increment counter every clock cycle if there is input
	...
	when(new_input_log){
		...
		rb.entries(entry_idx).result := 0.U.asTypeOf(new PositResult)
	}
	...
	pe.io.request.bits.num1 := rb.entries(chosen).request.operands(0).value(Nbits-1,0)
	pe.io.request.bits.num2 := rb.entries(chosen).request.operands(1).value(Nbits-1,0)
	pe.io.request.bits.num3 := rb.entries(chosen).request.operands(2).value(Nbits-1,0)
		...
		for(i <- 0 until NumFPGAEntries){
			for(j <- 0 until NumOperand){
				...
				val v_bit = MuxLookup(rb.entries(i).request.operands(j).value(BitsForOffset-1,0), 0.U, 
																(0 until NumOperandPerCacheline).toArray.map(x => (x.U ->  io.mem_read.data(x*ReadGranularity*8+Nbits+1))))
				val id_bit = MuxLookup(rb.entries(i).request.operands(j).value(BitsForOffset-1,0), 0.U, 
																(0 until NumOperandPerCacheline).toArray.map(x => (x.U ->  io.mem_read.data(x*ReadGranularity*8+Nbits))))
				when(rb.entries(i).request.operands(j).mode(1) === 1.U){
					when(tag === io.mem_read.resp_tag
						...
						rb.entries(i).request.operands(j).value := MuxLookup(rb.entries(i).request.operands(j).value(BitsForOffset-1,0), 0.U, 
																(0 until NumOperandPerCacheline).toArray.map(x => (x.U -> io.mem_read.data(x*ReadGranularity*8+Nbits-1, x*ReadGranularity*8 ))))
						rb.entries(i).request.operands(j).mode := 0.U
