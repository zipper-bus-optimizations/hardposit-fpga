...
/* common /main_flt_mem/ */
static Hardposit v[NA+1+1];	/* v[1:NA+1] */
static Hardposit aelt[NZ+1];	/* aelt[1:NZ] */
static Hardposit a[NZ+1];		/* a[1:NZ] */
static Hardposit x[NA+2+1];	/* x[1:NA+2] */
static Hardposit z[NA+2+1];	/* z[1:NA+2] */
static Hardposit p[NA+2+1];	/* p[1:NA+2] */
static Hardposit q[NA+2+1];	/* q[1:NA+2] */
static Hardposit r[NA+2+1];	/* r[1:NA+2] */
...
static void conj_grad (int colidx[], int rowstr[], Hardposit x[], Hardposit z[],
		Hardposit a[], Hardposit p[], Hardposit q[], Hardposit r[],
		//double w[],
		Hardposit *rnorm);
static void makea(int n, int nz, Hardposit a[], int colidx[], int rowstr[],
		int nonzer, int firstrow, int lastrow, int firstcol,
		int lastcol, Hardposit rcond, int arow[], int acol[],
		Hardposit aelt[], Hardposit v[], int iv[], Hardposit shift );
static void sparse(Hardposit a[], int colidx[], int rowstr[], int n,
		int arow[], int acol[], Hardposit aelt[],
		int firstrow, int lastrow,
		Hardposit x[], boolean mark[], int nzloc[], int nnza);
static void sprnvc(int n, int nz, Hardposit v[], int iv[], int nzloc[],
		int mark[]);
static int icnvrt(double x, int ipwr2);
static void vecset(int n, Hardposit v[], int iv[], int *nzv, int i, Hardposit val);
...
int main(int argc, char **argv) {
  init_accel();
	...
	Hardposit zeta;
	Hardposit rnorm;
	Hardposit norm_temp11;
	Hardposit norm_temp12;
	double t, mflops;
	char problem_class;
	boolean verified;
	...
	if (NA == 1400 && NONZER == 7 && SHIFT == 10.0) {
	//if (NA == 1400 && NONZER == 7 && NITER == 15 && SHIFT == 10.0) {
		problem_class = 'S';
		zeta_verify_value = 8.5971775078648;
	} else if (NA == 7000 && NONZER == 8 && SHIFT == 12.0) {
	//} else if (NA == 7000 && NONZER == 8 && NITER == 15 && SHIFT == 12.0) {
		problem_class = 'W';
		zeta_verify_value = 10.362595087124;
	} else if (NA == 14000 && NONZER == 11 && SHIFT == 20.0) {
	//} else if (NA == 14000 && NONZER == 11 && NITER == 15 && SHIFT == 20.0) {
		problem_class = 'A';
		zeta_verify_value = 17.130235054029;
	} else if (NA == 75000 && NONZER == 13 && SHIFT == 60.0) {
	//} else if (NA == 75000 && NONZER == 13 && NITER == 75 && SHIFT == 60.0) {
		problem_class = 'B';
		zeta_verify_value = 22.712745482631;
	} else if (NA == 150000 && NONZER == 15 && SHIFT == 110.0) {
	...
	makea(naa, nzz, a, colidx, rowstr, NONZER,
			...
			RCOND, arow, acol, aelt, v, iv, hp32(SHIFT));
	...
	for (it = 1; it <= NITER; it++) {
		...
		printf("    %5d       %20.14e%20.13e%20.13e\n", it, rnorm.toDouble(), zeta.toDouble(), fabsq((__float128)zeta.toDouble()-(__float128)zeta_verify_value));
	...
	epsilon = 1.0e-10;
	if (problem_class != 'U') {
		if (fabs(zeta.toDouble() - zeta_verify_value) <= epsilon) {
			verified = TRUE;
			printf(" VERIFICATION SUCCESSFUL\n");
			printf(" Zeta is    %20.12e\n", zeta.toDouble());
			printf(" Error is   %20.12e\n", zeta.toDouble() - zeta_verify_value);
		} else {
			verified = FALSE;
	...
	c_print_results("CG", problem_class, NA, 0, 0, NITER, nthreads, t, 
  ...
  c---------------------------------------------------------------------*/
{
	...
	Hardposit d, sum, rho, rho0, alpha, beta;
	int i, j, k;
	int cgit, cgitmax = 25;
	...
	for (cgit = 1; cgit <= cgitmax; cgit++) {
		...
		if(callcount>25) {
			compute_residual<Hardposit>(callcount-25, a, z, x, NA, firstrow, lastrow, firstcol, lastcol, rowstr, colidx);
		}
		...
		Hardposit shift )
{
	...
	Hardposit size, ratio, scale;
	int jcol;

	size = 1.0;
	ratio = pow(rcond.toDouble(), 1.0 / (double)n);
	nnza = 0;
