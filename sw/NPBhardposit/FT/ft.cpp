...
static void evolve(pcomplex u0[NZ][NY][NX], pcomplex u1[NZ][NY][NX],
		int t, int indexmap[NZ][NY][NX], int d[3]);
static void compute_initial_conditions(pcomplex u0[NZ][NY][NX], int d[3]);
...
static void fft(int dir, pcomplex x1[NZ][NY][NX], pcomplex x2[NZ][NY][NX]);
static void cffts1(int is, int d[3], pcomplex x[NZ][NY][NX],
		pcomplex xout[NZ][NY][NX],
		pcomplex y0[NX][FFTBLOCKPAD],
		pcomplex y1[NX][FFTBLOCKPAD]);
static void cffts2(int is, int d[3], pcomplex x[NZ][NY][NX],
		pcomplex xout[NZ][NY][NX],
		pcomplex y0[NX][FFTBLOCKPAD],
		pcomplex y1[NX][FFTBLOCKPAD]);
static void cffts3(int is, int d[3], pcomplex x[NZ][NY][NX],
		pcomplex xout[NZ][NY][NX],
		pcomplex y0[NX][FFTBLOCKPAD],
		pcomplex y1[NX][FFTBLOCKPAD]);
static void fft_init (int n);
static void cfftz (int is, int m, int n, pcomplex x[NX][FFTBLOCKPAD],
		pcomplex y[NX][FFTBLOCKPAD]);
static void fftz2 (int is, int l, int m, int n, int ny, int ny1,
		pcomplex u[NX], pcomplex x[NX][FFTBLOCKPAD],
		pcomplex y[NX][FFTBLOCKPAD]);
static int ilog2(int n);
static void checksum(int i, pcomplex u1[NZ][NY][NX], int d[3]);
static void verify (int d1, int d2, int d3, int nt,
		boolean *verified, char *problem_class);
...
int main(int argc, char **argv) {
  ...
  init_accel();
	...
	/*--------------------------------------------------------------------
	  ...
	  c-------------------------------------------------------------------*/
	static pcomplex u0[NZ][NY][NX];
	static pcomplex pad1[3];
	static pcomplex u1[NZ][NY][NX];
	static pcomplex pad2[3];
	static pcomplex u2[NZ][NY][NX];
	static pcomplex pad3[3];
	...
	static qcomplex timestep_truth[NZ][NY][NX];
	static pcomplex truth[NZ][NY][NX];
	compute_initial_conditions(truth, dims[0]);

	fft_init (dims[0][0]);

	fft(1, u1, u0);
	fft(-1, u0, u1);
	scale(NZ*NY*NX, u1, u1);
	compute_error<pcomplex>(&u1[0][0][0], &truth[0][0][0], NZ*NY*NX);
	...
	load_array<qcomplex>("./../FT_truth", "FT", "quad", problem_class, 0, &(timestep_truth[0][0][0]), NZ*NY*NX);
	compute_error<pcomplex, qcomplex>(&u0[0][0][0], &timestep_truth[0][0][0], NZ*NY*NX);
	...
	for (iter = 1; iter <= niter; iter++) {
		...
		if(iter==1) {
			load_array<qcomplex>("./../FT_truth", "FT", "quad", problem_class, 1, &(timestep_truth[0][0][0]), NZ*NY*NX);
			compute_error<pcomplex, qcomplex>(&u1[0][0][0], &timestep_truth[0][0][0], NZ*NY*NX);
		}
		if (TIMERS_ENABLED == TRUE) {  
			timer_start(T_FFT);
		}
		...
		if(iter==1) {
			load_array<qcomplex>("./../FT_truth", "FT", "quad", problem_class, 2, &(timestep_truth[0][0][0]), NZ*NY*NX);
			compute_error<pcomplex, qcomplex>(&u2[0][0][0], &timestep_truth[0][0][0], NZ*NY*NX);
		}
	}

	verify(NX, NY, NZ, niter, &verified, &problem_class);
	...
	c_print_results("FT", problem_class, NX, NY, NZ, niter, nthreads,
			total_time, mflops, "  Hardposit floating point", verified, 
...
static void fftz2 (int is, int l, int m, int n, int ny, int ny1,
	...
	for (i = 0; i < li; i++) {
		...
		for (k = 0; k < lk; k++) {
			for (j = 0; j < ny; j++) {
				Hardposit x11real, x11imag;
				Hardposit x21real, x21imag;
...
static void checksum(int i, pcomplex u1[NZ][NY][NX], int d[3]) {
	...
	sums[i].real += chk.real.toDouble();
	sums[i].imag += chk.imag.toDouble();
	/* complex % real */
