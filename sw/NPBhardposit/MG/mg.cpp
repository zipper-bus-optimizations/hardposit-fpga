...
static void mg3P(Hardposit ****u, Hardposit ***v, Hardposit ****r, Hardposit a[4],
		Hardposit c[4], int n1, int n2, int n3, int k);
static void psinv( Hardposit ***r, Hardposit ***u, int n1, int n2, int n3,
		Hardposit c[4], int k);
static void resid( Hardposit ***u, Hardposit ***v, Hardposit ***r,
		int n1, int n2, int n3, Hardposit a[4], int k );
static void rprj3( Hardposit ***r, int m1k, int m2k, int m3k,
		Hardposit ***s, int m1j, int m2j, int m3j, int k );
static void interp( Hardposit ***z, int mm1, int mm2, int mm3,
		Hardposit ***u, int n1, int n2, int n3, int k );
static void norm2u3(Hardposit ***r, int n1, int n2, int n3,
		Hardposit *rnm2, Hardposit *rnmu, int nx, int ny, int nz);
static void rep_nrm(Hardposit ***u, int n1, int n2, int n3,
		char *title, int kk);
static void comm3(Hardposit ***u, int n1, int n2, int n3, int kk);
static void zran3(Hardposit ***z, int n1, int n2, int n3, int nx, int ny, int k);
static void showall(Hardposit ***z, int n1, int n2, int n3);
static double power( double a, int n );
static void bubble( Hardposit ten[M][2], int j1[M][2], int j2[M][2],
		int j3[M][2], int m, int ind );
static void zero3(Hardposit ***z, int n1, int n2, int n3);
static void nonzero(Hardposit ***z, int n1, int n2, int n3);
...
int main(int argc, char *argv[]) {
  init_accel();
	...
	if (problem_class == 'A' || problem_class == 'S' || problem_class =='W') {
		/*--------------------------------------------------------------------
	...
	u = (Hardposit ****)malloc((lt+1)*sizeof(Hardposit ***));
	for (l = lt; l >=1; l--) {
		u[l] = (Hardposit ***)malloc(m3[l]*sizeof(Hardposit **));
		for (k = 0; k < m3[l]; k++) {
			u[l][k] = (Hardposit **)malloc(m2[l]*sizeof(Hardposit *));
			for (j = 0; j < m2[l]; j++) {
				u[l][k][j] = (Hardposit *)malloc(m1[l]*sizeof(Hardposit));
			}
		}
	}
	v = (Hardposit ***)malloc(m3[lt]*sizeof(Hardposit **));
	for (k = 0; k < m3[lt]; k++) {
		v[k] = (Hardposit **)malloc(m2[lt]*sizeof(Hardposit *));
		for (j = 0; j < m2[lt]; j++) {
			v[k][j] = (Hardposit *)malloc(m1[lt]*sizeof(Hardposit));
		}
	}
	r = (Hardposit ****)malloc((lt+1)*sizeof(Hardposit ***));
	for (l = lt; l >=1; l--) {
		r[l] = (Hardposit ***)malloc(m3[l]*sizeof(Hardposit **));
		for (k = 0; k < m3[l]; k++) {
			r[l][k] = (Hardposit **)malloc(m2[l]*sizeof(Hardposit *));
			for (j = 0; j < m2[l]; j++) {
				r[l][k][j] = (Hardposit *)malloc(m1[l]*sizeof(Hardposit));
			}
		}
	}
	...
	printf(" Size: %3dx%3dx%3d (problem_class %1c)\n",
			nx[lt], ny[lt], nz[lt], problem_class);
	...
	if (problem_class != 'U') {
		if (problem_class == 'S') {
			verify_value = 0.530770700573e-04;
		} else if (problem_class == 'W') {
			verify_value = 0.250391406439e-17;  /* 40 iterations*/
			/*				0.183103168997d-044 iterations*/
		} else if (problem_class == 'A') {
			verify_value = 0.2433365309e-5;
		} else if (problem_class == 'B') {
			verify_value = 0.180056440132e-5;
		} else if (problem_class == 'C') {
			verify_value = 0.570674826298e-06;
		}

		if ( fabs( rnm2.toDouble() - verify_value ) <= epsilon ) {
			verified = TRUE;
			printf(" VERIFICATION SUCCESSFUL\n");
			printf(" L2 Norm is %20.12e\n", rnm2.toDouble());
			printf(" Error is   %20.12e\n", rnm2.toDouble() - verify_value);
		} else {
			verified = FALSE;
	...
	c_print_results("MG", problem_class, nx[lt], ny[lt], nz[lt], 
	...
	for (j3 = 1; j3 < m3j-1; j3++) {
		...
		for (j2 = 1; j2 < m2j-1; j2++) {
			...
			for (j1 = 1; j1 < m1j-1; j1++) {
				...
				s[j3][j2][j1] =
					hp32(0.5) * r[i3+1][i2+1][i1+1]
					+ hp32(0.25) * ( r[i3+1][i2+1][i1] + r[i3+1][i2+1][i1+2] + x2)
					+ hp32(0.125) * ( x1[i1] + x1[i1+2] + y2)
					+ hp32(0.0625) * ( y1[i1] + y1[i1+2] );
			}
		}
	}
			...
			for (i2 = 0; i2 < mm2-1; i2++) {
				...
				for (i1 = 0; i1 < mm1-1; i1++) {
					...
					u[2*i3][2*i2][2*i1+1] = u[2*i3][2*i2][2*i1+1]
						+hp32(0.5)*(z[i3][i2][i1+1]+z[i3][i2][i1]);
				}
				for (i1 = 0; i1 < mm1-1; i1++) {
					u[2*i3][2*i2+1][2*i1] = u[2*i3][2*i2+1][2*i1]
						+hp32(0.5) * z1[i1];
					u[2*i3][2*i2+1][2*i1+1] = u[2*i3][2*i2+1][2*i1+1]
						+hp32(0.25)*( z1[i1] + z1[i1+1] );
				}
				for (i1 = 0; i1 < mm1-1; i1++) {
					u[2*i3+1][2*i2][2*i1] = u[2*i3+1][2*i2][2*i1]
						+hp32(0.5) * z2[i1];
					u[2*i3+1][2*i2][2*i1+1] = u[2*i3+1][2*i2][2*i1+1]
						+hp32(0.25)*( z2[i1] + z2[i1+1] );
				}
				for (i1 = 0; i1 < mm1-1; i1++) {
					u[2*i3+1][2*i2+1][2*i1] = u[2*i3+1][2*i2+1][2*i1]
						+hp32(0.25)* z3[i1];
					u[2*i3+1][2*i2+1][2*i1+1] = u[2*i3+1][2*i2+1][2*i1+1]
						+hp32(0.125)*( z3[i1] + z3[i1+1] );
				}
			}
		}
	} else {
			...
			for ( i2 = d2; i2 <= mm2-1; i2++) {
				...
				for ( i1 = 1; i1 <= mm1-1; i1++) {
					u[2*i3-d3-1][2*i2-d2-1][2*i1-t1-1] =
						u[2*i3-d3-1][2*i2-d2-1][2*i1-t1-1]
						+hp32(0.5)*(z[i3-1][i2-1][i1]+z[i3-1][i2-1][i1-1]);
				}
			}
			for ( i2 = 1; i2 <= mm2-1; i2++) {
				for ( i1 = d1; i1 <= mm1-1; i1++) {
					u[2*i3-d3-1][2*i2-t2-1][2*i1-d1-1] =
						u[2*i3-d3-1][2*i2-t2-1][2*i1-d1-1]
						+hp32(0.5)*(z[i3-1][i2][i1-1]+z[i3-1][i2-1][i1-1]);
				}
				for ( i1 = 1; i1 <= mm1-1; i1++) {
					u[2*i3-d3-1][2*i2-t2-1][2*i1-t1-1] =
						u[2*i3-d3-1][2*i2-t2-1][2*i1-t1-1]
						+hp32(0.25)*(z[i3-1][i2][i1]+z[i3-1][i2-1][i1]
			...
			for ( i2 = d2; i2 <= mm2-1; i2++) {
				for ( i1 = d1; i1 <= mm1-1; i1++) {
					u[2*i3-t3-1][2*i2-d2-1][2*i1-d1-1] =
						u[2*i3-t3-1][2*i2-d2-1][2*i1-d1-1]
						+hp32(0.5)*(z[i3][i2-1][i1-1]+z[i3-1][i2-1][i1-1]);
				}
				for ( i1 = 1; i1 <= mm1-1; i1++) {
					u[2*i3-t3-1][2*i2-d2-1][2*i1-t1-1] =
						u[2*i3-t3-1][2*i2-d2-1][2*i1-t1-1]
						+hp32(0.25)*(z[i3][i2-1][i1]+z[i3][i2-1][i1-1]
			...
			for ( i2 = 1; i2 <= mm2-1; i2++) {
				for ( i1 = d1; i1 <= mm1-1; i1++) {
					u[2*i3-t3-1][2*i2-t2-1][2*i1-d1-1] =
						u[2*i3-t3-1][2*i2-t2-1][2*i1-d1-1]
						+hp32(0.25)*(z[i3][i2][i1-1]+z[i3][i2-1][i1-1]
								+z[i3-1][i2][i1-1]+z[i3-1][i2-1][i1-1]);
				}
				for ( i1 = 1; i1 <= mm1-1; i1++) {
					u[2*i3-t3-1][2*i2-t2-1][2*i1-t1-1] =
						u[2*i3-t3-1][2*i2-t2-1][2*i1-t1-1]
						+hp32(0.125)*(z[i3][i2][i1]+z[i3][i2-1][i1]
...
static void rep_nrm(Hardposit ***u, int n1, int n2, int n3,
	...
	printf(" Level%2d in %8s: norms =%20.12e%20.12e\n",
			kk, title, rnm2.toDouble(), rnmu.toDouble());
}
...
static void showall(Hardposit ***z, int n1, int n2, int n3) {
	...
	for (i3 = 0; i3 < m3; i3++) {
		for (i1 = 0; i1 < m1; i1++) {
			for (i2 = 0; i2 < m2; i2++) {
				printf("%6.3f", z[i3][i2][i1].toDouble());
			}
			printf("\n");
		}
		printf(" - - - - - - - \n");
