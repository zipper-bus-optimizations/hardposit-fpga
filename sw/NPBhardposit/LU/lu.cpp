/*--------------------------------------------------------------------
  ...
  eAS Parallel Benchmarks 3.0 structured OpenMP C versions - LU
...
static void blts (int nx, int ny, int nz, int k,
		...
		Hardposit ldz[ISIZ1][ISIZ2][5][5],
		Hardposit ldy[ISIZ1][ISIZ2][5][5],
		Hardposit ldx[ISIZ1][ISIZ2][5][5],
		Hardposit d[ISIZ1][ISIZ2][5][5],
		int ist, int iend, int jst, int jend,
		int nx0, int ny0 );
static void buts(int nx, int ny, int nz, int k,
		Hardposit omega,
		Hardposit v[ISIZ1][ISIZ2/2*2+1][ISIZ3/2*2+1][5],
		Hardposit tv[ISIZ1][ISIZ2][5],
		Hardposit d[ISIZ1][ISIZ2][5][5],
		Hardposit udx[ISIZ1][ISIZ2][5][5],
		Hardposit udy[ISIZ1][ISIZ2][5][5],
		Hardposit udz[ISIZ1][ISIZ2][5][5],
		int ist, int iend, int jst, int jend,
		int nx0, int ny0 );
...
int main(int argc, char **argv) {
  init_accel();
	...
	/*--------------------------------------------------------------------
	  c   verification test
	  --------------------------------------------------------------------*/
	verify ( rsdnm, errnm, frc, &problem_class, &verified );
	...
	c_print_results("LU", problem_class, nx0,
...
static void erhs(void) {
	...
	Hardposit  q;
	Hardposit  u21, u31, u41;
	Hardposit  tmp;
	Hardposit  u21i, u31i, u41i, u51i;
	Hardposit  u21j, u31j, u41j, u51j;
	Hardposit  u21k, u31k, u41k, u51k;
	Hardposit  u21im1, u31im1, u41im1, u51im1;
	Hardposit  u21jm1, u31jm1, u41jm1, u51jm1;
	Hardposit  u21km1, u31km1, u41km1, u51km1;
...
static void error(void) {
	...
	for (m = 0; m < 5; m++) {
		//errnm[m] = sqrtq( errnm[m] / ( (nx0-2)*(ny0-2)*(nz0-2) ) );
		errnm[m] = sqrtq( errnm[m] );
	}
}
...
/*--------------------------------------------------------------------
  --------------------------------------------------------------------*/
static void l2norm_quad (int nx0, int ny0, int nz0,
		int ist, int iend,
		int jst, int jend,
		/*--------------------------------------------------------------------
		  c   To improve cache performance, second two dimensions padded by 1 
...
static void pintgr(void) {
	...
	Hardposit phi1[ISIZ2+2][ISIZ3+2];	/* phi1(0:isiz2+1,0:isiz3+1) */
	Hardposit phi2[ISIZ2+2][ISIZ3+2];	/* phi2(0:isiz2+1,0:isiz3+1) */
	Hardposit  frc1, frc2, frc3;
...
static void read_input(void) {
	...
	if (fp != NULL) {
		...
		fscanf(fp, "%lf%lf%lf%lf%lf",
				&tmp_tolrsd[0], &tmp_tolrsd[1], &tmp_tolrsd[2], &tmp_tolrsd[3], &tmp_tolrsd[4]);
		tolrsd[0] = tmp_tolrsd[0]; tolrsd[1] = tmp_tolrsd[1]; tolrsd[2] = tmp_tolrsd[2]; tolrsd[3] = tmp_tolrsd[3]; tolrsd[4] = tmp_tolrsd[4];
		while(fgetc(fp) != '\n');
...
static void setiv(void) {
	...
	int iglob, jglob;
	Hardposit  xi, eta, zeta;
	Hardposit  pxi, peta, pzeta;
	Hardposit  ue_1jk[5],ue_nx0jk[5],ue_i1k[5],
...
static void ssor(void) {
	...
	int istep;
	Hardposit  tmp;
	Hardposit  delunm[5], tv[ISIZ1][ISIZ2][5];
...
static void verify(Hardposit xcr[5], __float128 xce[5], Hardposit xci,
	...
	__float128 xcrref[5],xceref[5],xciref, 
	...
	*verified = TRUE;
	...
	if (*problem_class != 'U') {
		printf("\n Verification being performed for problem_class %1c\n", *problem_class);
	...
	for (m = 0; m < 5; m++) {
		quadmath_snprintf(quad_str[1], sizeof quad_str[0], "%20.13Qe", xcrref[m]);
		quadmath_snprintf(quad_str[2], sizeof quad_str[0], "%20.13Qe", xcrdif[m]);
		if (*problem_class  ==  'U') {
			printf("          %2d  %20.13e\n", m, xcr[m].toDouble());
		} else if (xcrdif[m] > epsilon) {
			*verified = FALSE;
			printf(" FAILURE: %2d  %20.13e%s%s\n",
					m, xcr[m].toDouble(), quad_str[1], quad_str[2]);
		} else {
			printf("          %2d  %20.13e%s%s\n",
					m, xcr[m].toDouble(), quad_str[1], quad_str[2]);
		}
	}
	...
	for (m = 0; m < 5; m++) {
		...
		if (*problem_class  ==  'U') {
			...
			*verified = FALSE;
			printf(" FAILURE: %2d  %se%s%s\n",
					m, quad_str[0], quad_str[1], quad_str[2]);
		} else {
			printf("          %2d  %s%s%s\n",
					m, quad_str[0], quad_str[1], quad_str[2]);
		}
	...
	quadmath_snprintf(quad_str[1], sizeof quad_str[0], "%20.13Qe", xciref);
	quadmath_snprintf(quad_str[2], sizeof quad_str[0], "%20.13Qe", xcidif);
	if (*problem_class  ==  'U') {
		printf("              %20.13e\n", xci.toDouble());
	} else if (xcidif > epsilon) {
		*verified = FALSE;
		printf(" FAILURE:     %20.13e%s%s\n", 
				xci.toDouble(), quad_str[1], quad_str[2]);
	} else {
		printf("              %20.13e%s%s\n",
				xci.toDouble(), quad_str[1], quad_str[2]);
	}
